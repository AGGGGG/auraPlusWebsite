...
(function() {
  ...
  this.AudioPlayerUI = (function() {
    ...
    function AudioPlayerUI(options) {
      ...
      //this.goToSong(0);
        if (localStorage.getItem("lastSongName")) {
            //alert("LOCAL STORAGE WORKED");
            for (var i = 0; i < this.songs.length; i++) {
                //var test = this.songs[i].name; //testing only
                //var test2 = localStorage.getItem("lastSongName"); //testing puropses only
                if (this.songs[i].name === localStorage.getItem("lastSongName")) {
                    this.goToSong(i);
                    var audioEl = document.getElementById("audio-player").childNodes[5];
                    audioEl.currentTime = localStorage.getItem("lastTime");
                    break;
                }
            }
        } else {
    ...
    songs: [
      {
        ...
        srcs: [
          {
            src: "audio/Kiss Me Thru the Phone.mp3",
            type: "audio/mp3"
          }
        ]
      }, {
          image: "",
          name: "Theres A Lot Going On",
          srcs: [
              {
                  src: "audio/Theres A Lot Going On.mp3",
                  type: "audio/mp3"
              }
          ]
      }
...
window.onunload = function() {
    //TESTING
        //console.log("CONSOLE.LOG CALLED"); //works
        //childNodes[5] is audio element
        //console.log(document.getElementById("audio-player").childNodes[5].childNodes[0].getAttribute("src")); //WORKS --- GETS NAME OF AUDIO FILE SOURCE
    //TESTING
    /*
    console.log("should be audio-player: " + document.getElementById("audio-player"));//works
    console.log("should be audio element: " + document.getElementById("audio-player").childNodes[5]);//works
    console.log(document.getElementById("audio-player").childNodes[5].childNodes);
    console.log()
    console.log("should be source element: " + document.getElementById("audio-player").childNodes[0]);
    console.log("SHOULD BE SOURCE: " + sourceEl);
    console.log("SRC??: " + sourceEl.getAttribute("src"));
    console.log("SONGNAME: " + songName); //WORKS -- GETS SONG NAME
    console.log("currentTime: " + audioEl.currentTime); //WORKS
    */
    //IT WORKS!!!!
        var audioEl = document.getElementById("audio-player").childNodes[5];
        var sourceEl = audioEl.firstChild;
        var sourceName = sourceEl.getAttribute("src");
        var songName = sourceName.substring(6, sourceName.length-4);
        localStorage.setItem("lastSongName", songName);
        localStorage.setItem("lastTime", audioEl.currentTime);
        console.log(localStorage.getItem("lastSongName") + ", " + localStorage.getItem("lastTime")); //WORKS
    }